...
- [ ] Support multi-line input with a trailing `\` continuation or a `/paste` command that opens `@inquirer/editor` (`typedPrompts` already drops multi-line entries from history). Test that `@src/index.ts` expands to its line-numbered content and that ignored paths are never offered for completion
